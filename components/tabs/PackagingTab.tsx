...
import { packagingPresets, presets } from '../../data/presets';
import { BrandGuidelines } from '../../types';

// --- Reusable Helper Components from ImageGenerationTab ---
...
const placeholderPrompts = [
    "A cardboard packaging box for a luxury product. The box is sleek and matte black, with 'Graphite-AdGeni' in silver foil typography on the label. The label wraps around the container.",
    "A clear glass bottle for an artisanal gin, 'Juniper Grove'. The label is thick, textured paper with a botanical illustration of juniper berries and a simple, elegant serif font. The bottle is filled and has a dark wooden stopper.",
    "A resealable stand-up pouch for organic coffee beans. The packaging is made of a matte, kraft paper material. The design is modern and clean, with a transparent window showing the beans. 'Morning Ritual' is printed in a bold, friendly font.",
    "An aluminum can for a craft beer named 'Cosmic Haze IPA'. The can has a full-wrap label with a vibrant, psychedelic illustration of swirling galaxies and hops. The finish is a soft-touch matte.",
    "A minimalist white tube for a high-end face cream, 'Etherea'. The branding is subtle, with the product name in a light grey, small, sans-serif font. It has a silver cap and stands against a soft pastel background.",
    "A luxury chocolate bar packaging. The wrapper is dark, forest green paper with an intricate gold foil pattern of cocoa leaves. The brand name 'Melt' is embossed in the center. The bar is shown on a dark slate surface."
];

// --- Main Packaging Tab Component ---

interface PackagingTabProps {
    onUseAsProduct: (imageUrl: string) => void;
    brandGuidelines: BrandGuidelines;
    imageModel: string;
}

const PackagingTab: React.FC<PackagingTabProps> = ({ onUseAsProduct, brandGuidelines, imageModel }) => {
    const [prompt, setPrompt] = useState('');
    const [placeholder, setPlaceholder] = useState(placeholderPrompts[0]);
    const [selectedPresets, setSelectedPresets] = useState({ packagingType: 'box', material: 'cardboard', style: 'minimalist', color: 'none', mood: 'none' });
    ...
    const buildFinalPrompt = useCallback(() => {
        const basePrompt = "Professional product photography of a single item on a clean, neutral studio background, commercial packshot, soft lighting, sharp focus.";
        
        const stylePrompt = selectedPresets.style === 'use_brand_guidelines' 
            ? (brandGuidelines.styles ? `, ${brandGuidelines.styles}` : '')
            : getSafePrompt(packagingPresets.labelStyle, selectedPresets.style);
        ...
        const packagingTypePrompt = getSafePrompt(packagingPresets.packagingType, selectedPresets.packagingType);
        const materialPrompt = getSafePrompt(packagingPresets.material, selectedPresets.material);

        return `${prompt}${packagingTypePrompt}${materialPrompt}${stylePrompt}${colorPrompt}${moodPrompt}, ${basePrompt}`;
    }, [prompt, selectedPresets, brandGuidelines]);


    const handleGenerate = async () => {
        if (!prompt) { setError('Please enter a description for your packaging.'); return; }
        setError(null); setIsLoading(true); setImages([]); setSelectedImages(new Set());
        
        try {
            const finalPrompt = buildFinalPrompt();
            const result = await generateAdImages(finalPrompt, [], numImages, imageModel);
            setImages(result);
        } catch (e: unknown) {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            Array.from(selectedImages).forEach((imageUrl, i) => {
                ...
                if (blob) zip.file(`packaging-image-${i + 1}.${blob.type.split('/')[1] || 'png'}`, blob);
            });
            const content = await zip.generateAsync({ type: 'blob' });
            saveAs(content, 'packaging-design-images.zip');
        } catch (e) {
            const errorMessage = e instanceof Error ? e.message : "Could not create the zip file.";
            setError(errorMessage);
        } finally {
        ...
        <div className="flex flex-col h-full bg-gray-200">
            ...
            <main className="flex-1 flex flex-col p-6 overflow-hidden">
                <header className="flex justify-between items-center mb-4 flex-shrink-0">
                    <div>
                        <h3 className="text-lg font-bold text-gray-800">Generated Packaging Concepts</h3>
                        <p className="text-sm text-gray-500">{images.length > 0 ? `${images.length} concept${images.length === 1 ? '' : 's'} generated. Select concepts to download.` : 'Your generated packaging concepts will appear here.'}</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    {isLoading ? <div className="h-full flex items-center justify-center"><LoadingSpinner message="Our AI is designing your packaging..." /></div>
                    : (
                        images.length > 0 ? (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                                {images.map((src, i) => (
                                    <div key={i} className="relative group aspect-square bg-gray-200 rounded-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 shadow-[5px_5px_10px_#d1d5db,-5px_-5px_10px_#ffffff] hover:shadow-[8px_8px_16px_#d1d5db,-8px_-8px_16px_#ffffff]">
                                        <img src={src} className="w-full h-full object-cover cursor-pointer" alt={`Generated packaging image ${i+1}`} onClick={() => { setActiveImage(src); setIsModalOpen(true); }}/>
                                        ...
                                        <div onClick={(e) => { e.stopPropagation(); toggleImageSelection(src); }} title="Select image" className={`absolute top-3 right-3 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-200 border-2 z-10 cursor-pointer ${selectedImages.has(src) ? 'bg-blue-500 border-blue-600 scale-110' : 'bg-white/70 border-gray-300 group-hover:bg-white group-hover:scale-110'}`}>
                                         ...
                                         <button onClick={() => onUseAsProduct(src)} className="absolute bottom-3 right-3 bg-blue-500/90 backdrop-blur-sm text-white px-3 py-1 text-xs font-bold rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-blue-600">
                                            Use as Product
                                        </button>
                                    </div>
                                ))}
                             ...
                             <div className="h-full flex items-center justify-center">
                                <div className="text-center text-gray-400 p-12 rounded-2xl shadow-[inset_5px_5px_10px_#d1d5db,inset_-5px_-5px_10px_#ffffff]">
                                    ...
                                    <h3 className="text-lg font-semibold mb-2 text-gray-500">Packaging Design Studio</h3>
                                    <p>Describe your packaging and click Generate.</p>
                                </div>
                             </div>
                        )
                    )}
            ...
            <footer className="bg-gray-200 p-4 flex-shrink-0 z-10">
                <div className="p-3 bg-gray-200 rounded-lg shadow-[inset_3px_3px_6px_#d1d5db,inset_-3px_-3px_6px_#ffffff] mb-4">
                     <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <ControlSection title="Packaging Type">
                            <SelectMenu label="Type" value={selectedPresets.packagingType} onChange={(e) => handlePresetChange('packagingType', e.target.value)} options={packagingPresets.packagingType} />
                        </ControlSection>
                        <ControlSection title="Material">
                            <SelectMenu label="Material" value={selectedPresets.material} onChange={(e) => handlePresetChange('material', e.target.value)} options={packagingPresets.material} />
                        </ControlSection>
                        <ControlSection title="Label & Branding">
                            <SelectMenu showBrandOption label="Style" value={selectedPresets.style} onChange={(e) => handlePresetChange('style', e.target.value)} options={packagingPresets.labelStyle} />
                        </ControlSection>
