...
const ImageGenerationTab: React.FC<ImageGenerationTabProps> = ({ importedProductImage, onProductImportConsumed, importedSceneImage, onSceneImportConsumed, brandGuidelines, onEditImage, images, setImages, imageModel }) => {
        ...
        try {
            if (isSmartMode) {
                ...
                if (sceneImage) {
                    ...
                    const sceneResult = await generateAdImages(scenePrompt, [], 1, imageModel);
                ...
                const productResult = await generateAdImages(productPrompt, productAssets, 1, imageModel);
                ...
                const result = await generateAdImages(finalPrompt, driverInputs, numImages, imageModel);
                setImages(result);
                ...
                const result = await generateAdImages(finalPrompt, assets, numImages, imageModel);
                setImages(result);
            }
    ...
    const handleUpscaleImage = async (imageSrc: string) => {
        ...
        try {
            ...
            const upscaledUrl = await upscaleImage(imageInput, imageModel);
